    ...
    for (int ttl = 1; ttl <= MAX_HOPS; ) {
        ...
        while (1) {
            ...
            if (ret == 0) {
                ...
                int bytes = recvfrom(sockfd, recv_buf, PACKET_LEN, 0, (struct sockaddr *) &response_addr, &response_len);
                ...
                if (bytes >= (2 * (sizeof(ipheader) + sizeof (icmpheader)))) {
                    ...
                    if (router_icmp->icmp_type == ICMP_TIME_EXCEEDED) {
                        icmpheader * original_icmp = (icmpheader *) (recv_buf + (2 * sizeof (ipheader)) + sizeof (icmpheader));
                        if (original_icmp->icmp_seq == ttl && original_icmp->icmp_id == getpid()) {
                            cout << inet_ntoa(response_addr.sin_addr) << endl;
                            ttl++;
                            break;
                        }
                    }
                    ...
                    icmpheader * response_icmp = (icmpheader *) (recv_buf + (sizeof (ipheader)));
                    if (response_icmp->icmp_type == ICMP_ECHO_REPLY && response_icmp->icmp_id == getpid()) {
                        cout << inet_ntoa(response_addr.sin_addr) << endl;
                        return;
                    }
                }
