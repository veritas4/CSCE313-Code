...
int main (int argc, char* argv[]) {
	...
	while (true) {
		...
		TCPRequestChannel *handle_requests =  new TCPRequestChannel(newsockfd);
		requestChannels.push_back(handle_requests);

		thread loopthread(handle_process_loop, handle_requests);
		loopthread.detach();
	}
	for (auto i : requestChannels) {
		delete i;
	}
