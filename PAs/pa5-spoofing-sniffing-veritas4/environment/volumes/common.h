...
{
    udpheader *udp = (udpheader *)(ip + sizeof(ipheader));
    ...
    dest_info.sin_addr.s_addr = ip->iph_destip.s_addr;
    dest_info.sin_port = udp->udp_dport;

    if (connect(sockfd,(struct sockaddr*)&dest_info, sizeof(dest_info)) < 0) {
    ...
    if (sendto(sockfd, ip, ntohs(ip->iph_len), 0, (struct sockaddr*)&dest_info, sizeof(dest_info)) < 0) {
        ...
        cout << inet_ntoa(ip->iph_sourceip) << " --> " << inet_ntoa(ip->iph_destip) << endl;
    }
